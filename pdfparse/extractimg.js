...
let extractimgScrip = (pdf) => new Promise(async (resolve, reject) => {
  let originalPdfPath = `/tmp/${pdf}.pdf`;
  ...
  pdfDoc.index.index.forEach((pdfObject, ref) => {
    ...
    if (subtype === PDFName.from('Image')) {
      imagesInDoc.push({
        ...
        bitsPComponent: bitsPerComponent,
    ...
    if (err) console.error(err);
      ...
      for (const img of imagesInDoc) {
        if (!img.isAlphaLayer) {
          ...
          const tipo = img.type === 'png'? 'png' : 'jpg'   
          console.log('imagen de tipo: ' + img.type)       
          // fs.writeFileSync(`./pdfparse/images/out1.${pdf}.${idx}.${tipo}`, imageData); // guarga imagenes en carpeta
          imgs.push(`${idx}.${tipo}`)
          bufdata.push(imagesInDoc[idx].data)
          idx += 1;
        }
      }
      const img = {
...
module.exports = (pdf) => extractimgScrip(pdf)